...
- **`--command` repeats, and every value runs in the one session.** The commands run in the order given, each waiting for the prompt the previous one returned to, and a value is never split on a comma, so `TELEE_COMMAND` carries exactly one command. On `srx` the echo also shows the `| no-more` the script appends.
