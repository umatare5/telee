...
- **`[ERROR] Host key for <host> does not match known_hosts: it holds TYPE at FILE:LINE …`** — another type is on record for the host, and no remedy follows because another device could present it too. The case is common, as `ssh` records the ed25519 key it prefers while this client negotiates ECDSA or RSA first. The presented type is added only once its fingerprint matches the device's own, taken with `ssh-keyscan -t TYPE` where that reaches the device and `ssh-keygen -lf`, or it is pinned with `--host-key-path`.
