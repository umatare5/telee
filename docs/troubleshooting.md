...
Several `-C` values share the session, so a failure on any of them ends the run with nothing on stdout, whichever answered before it. A copy of the prompt that arrives ahead of the real one, as IOS reprints it after `?`, can end the step early, so the run can exit 0 with a transcript that ends before the last answer does.
