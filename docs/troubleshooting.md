...
- **`[ERROR] Host key for <host> does not match known_hosts: it holds TYPE at FILE:LINE …`** — another type is on record for the host, and no remedy follows because another device could present it too. The case is common, as `ssh` records the ed25519 key it prefers while this client negotiates ECDSA or RSA first. The presented type is added, or pinned with `--host-key-path`, only once `ssh-keygen -lf` gives its fingerprint for the key IOS prints under `show ip ssh`, or that `ssh-keyscan -t TYPE` takes.
